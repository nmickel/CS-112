...
class AllTests (unittest.TestCase):
	...
	# zigzag test - 35pts
	def test_zigzag_1  (self):self.assertEqual(zigzag('12345678'), '18273645')
	def test_zigzag_2  (self):self.assertEqual(zigzag('123456789'), '192837465')
	def test_zigzag_3  (self):self.assertEqual(zigzag('abcdefgh'), 'ahbgcfde')
	def test_zigzag_4  (self):self.assertEqual(zigzag('GeorgeMason'), 'GneoosragMe')
	def test_zigzag_5  (self):self.assertEqual(zigzag('gEorge mAsoN'), 'gNEoosrAgme ')
	def test_zigzag_6  (self):self.assertEqual(zigzag('my\tcl@ss\n'), 'm\nys\tsc@l')
	def test_zigzag_7  (self):self.assertEqual(zigzag('\"a-b-c-d-e-f\"'), '\"\"af--be--cd-')
	def test_zigzag_8  (self):self.assertEqual(zigzag('info [at] gmu.edu'), 'iundfeo. u[magt ]')
	def test_zigzag_9  (self):self.assertEqual(zigzag('135798642'), '123456789')
	def test_zigzag_10 (self):self.assertEqual(zigzag('G.eo\ros&rag_Me'), 'Ge.Me_og\raors&')
	def test_zigzag_11 (self):self.assertEqual(zigzag('#$%'), '#%$')
	def test_zigzag_12 (self):self.assertEqual(zigzag('&^'), '&^')
	def test_zigzag_13 (self):self.assertEqual(zigzag('_'), '_')
	def test_zigzag_14 (self):self.assertEqual(zigzag(''), '')
     
	# sum_gt_avg tests - 25pts
	def test_sum_gt_avg_1  (self):self.assertEqual(sum_gt_avg([]), 0)
	def test_sum_gt_avg_2  (self):self.assertEqual(sum_gt_avg([1,2,3,4,5]), 9)
	def test_sum_gt_avg_3  (self):self.assertEqual(sum_gt_avg([1,2,3,-4,5]), 10)
	def test_sum_gt_avg_4  (self):self.assertEqual(sum_gt_avg([-1,-2,-3,-4,-5]), -3)
	def test_sum_gt_avg_5  (self):self.assertEqual(sum_gt_avg([5]), 0)
	def test_sum_gt_avg_6  (self):self.assertEqual(sum_gt_avg([3, 3]), 0)
	def test_sum_gt_avg_7  (self):self.assertEqual(sum_gt_avg([-0.5, 0.1, 0.3, -1.8]), 0.4)
	def test_sum_gt_avg_8  (self):self.assertEqual(sum_gt_avg([2**-3, 2**-2, 2**-1, 2**0, 2**1, 2**2, 2**3, 2**4, 2**5]), 56)
	def test_sum_gt_avg_9  (self):self.assertEqual(sum_gt_avg([1/1, -1/2, 1/4, -1/5, 1/8]), 1.25)
	def test_sum_gt_avg_10 (self):self.assertEqual(sum_gt_avg([-0.40,0.42,0.25,-0.86,0.59,-0.19,-0.80,0.46,0.31,-0.37,0.84,0.74,0.59,-0.87,0.90,-0.68,0.41,0.01,-0.90,0.58]), 6.09)
     
	# count_pairs tests - 40pts
	def test_count_pairs_1  (self):self.assertEqual(count_pairs([1,2,3,4,5]), 4)
	def test_count_pairs_2  (self):self.assertEqual(count_pairs([1,2,3,1,0]), 7)
	def test_count_pairs_3  (self):self.assertEqual(count_pairs([3,1,2]), 1)
	def test_count_pairs_4  (self):self.assertEqual(count_pairs([3,13,4,5]), 0)
	def test_count_pairs_5  (self):self.assertEqual(count_pairs([0,0,0,0]), 6)
	def test_count_pairs_6  (self):self.assertEqual(count_pairs([1,2,3,5,7,11,13,17,19,23]), 5)
	def test_count_pairs_7  (self):self.assertEqual(count_pairs([1,2,2,3,5,7,11,13,17,19,23]), 10)
	def test_count_pairs_8  (self):self.assertEqual(count_pairs([23,9,13,26,17,22,25,7,-2,-4]), 7)
	def test_count_pairs_9  (self):self.assertEqual(count_pairs([12,22,-4,17, 8,24,-10,14,-3,-1]), 6)
	def test_count_pairs_10 (self):self.assertEqual(count_pairs([1,2,8,4,2,-3,-4,-3,4,-2]), 14)
	def test_count_pairs_11 (self):self.assertEqual(count_pairs([16,-1,13,30,24,-1,-4,-2,2,-5]), 4)
	def test_count_pairs_12 (self):self.assertEqual(count_pairs([-1,-51,-77,-33,-68,-17,66,67, 1,-7,39,-53,6,8,-41]), 8)
	def test_count_pairs_13 (self):self.assertEqual(count_pairs([72,-55,-52,-12,74,-23,-48,72,40,62,47,-13,74]), 3)
	def test_count_pairs_14 (self):self.assertEqual(count_pairs([8,6,3,6,-1,4,7]), 4)
	def test_count_pairs_15 (self):self.assertEqual(count_pairs([-7,-10,-1,-2,-3,-8,-5]), 7)
	def test_count_pairs_16 (self):self.assertEqual(count_pairs([-4,-1,-6,9,-1,-8,-1,-6,-8,1,-5,-3,9,7,8,9,8]), 42)
